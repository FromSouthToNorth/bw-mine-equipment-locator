...
**6. 反向编码约束**：候选名含 specific code（3+位纯数字 / 字母+数字 / 负数水平），但描述完全不含候选的任何 specific code → REJECT。
...
**7. `_LOCATION_SEMANTICS` 冲突**：描述含 `_LOCATION_SEMANTICS` 关键字但候选不含允许词时，直接拒绝（编码/LCS 无法掩盖）。|
